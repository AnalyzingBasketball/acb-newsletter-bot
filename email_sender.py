...
if first_line.startswith("ASUNTO:"):
    asunto_email = first_line.replace("ASUNTO:", "").strip()
    md_content = "\n".join(lines[1:]).strip()
else:
    asunto_email = "Informe Liga Endesa - Analyzing Basketball"
    md_content = raw_content

# --- 3. PREPARAR CAMPAÑA ---
print("📥 Preparando campaña de Email...", flush=True)
...
# --- 5. ENVÍO MASIVO ---
print(f"🚀 Iniciando envío a {len(lista_emails)} destinatarios...", flush=True)
...
try:
    ...
    for email in lista_emails:
        try:
            ...
            enviados += 1
            print(f"📨 Enviado a: {email}", flush=True)
    ...
    with open("data/email_error_log.txt", "w", encoding="utf-8") as ef:
        ef.write(err)
    sys.exit(1)
